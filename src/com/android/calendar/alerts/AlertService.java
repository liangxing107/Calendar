...
public class AlertService extends Service {
    ...
    void processMessage(Message msg) {
        ...
        if (!action.equals(android.provider.CalendarContract.ACTION_EVENT_REMINDER)
                ...
                && !action.equals(Utils.UPDATE_EVENT_REMINDER_ACTION)) {
    ...
    public static boolean updateAlertNotification(Context context) {
        ...
        try {
            while (alertCursor.moveToNext()) {
                ...
                ///M:Here modify notificationEventBegin <= beginTime && notificationEventStatus < newStatus@{
                ///to notificationEventBegin >= beginTime && notificationEventStatus <= newStatus
                ///So can selected the event that current create to show on the status bar. 
                ///@}
    ...
    private static void postNotification(Context context, SharedPreferences prefs,
        ...
        nullNotification.defaults |= Notification.DEFAULT_LIGHTS; 
        ...
        if (!quietUpdate) {
            ...
            /*
             * Before notify the event info normally, here we need to  notify a null notification here to clear the notification info!!. 
             * the nullNotification would not be pop up in status bar.
             * The purpose: show the reminder info in status bar when it already in the status bar.
             * Why: 1.If don't do it, it can not pop up the reminder info when it has been notified before.
             * 2.The google's default behavior is:don't show the reminder's info again when its content is the same as the one that has already stored in the status bar.
             * 
             * UI: it can pop up the reminder's info in the status bar when AlertReceiver receive a EVENT_REMINDER intents.
             */
            nm.notify(0, nullNotification);
            //send the normal notification with the same identifier to pop up what we want.
        }

        LogUtil.i(TAG, "start true notification: " + notification); 
