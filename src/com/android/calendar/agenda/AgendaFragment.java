...
public class AgendaFragment extends Fragment implements CalendarController.EventHandler,
    ...
    private static final String TAG = "Calendar";//AgendaFragment.class.getSimpleName();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ///M: add context menu to share event @{
        if((AllInOneActivity.mAgendaReqType == AllInOneActivity.REQ_TYPE_AGENDA_NULL)
        		&& Utils.hasImportCalendarProvider(getActivity()) ) {
            this.registerForContextMenu(mAgendaListView);
        }
        ///@}
...
//        Utils.setDefaultView(this, CalendarApplication.AGENDA_VIEW_ID);
    ...
    public long getSupportedEventTypes() {
        ///M:#ClearAllEvents# add a support type for clear all events--EVENTS_CLEARED.
        return EventType.GO_TO | EventType.EVENTS_CHANGED | ((mUsedForSearch)?EventType.SEARCH:0) | EventType.EVENTS_CLEARED;
    }
    ...
    public void handleEvent(EventInfo event) {
        if (event.eventType == EventType.GO_TO) {
            ...
            eventsChanged();
        } else if(event.eventType == EventType.EVENTS_CLEARED) {
            ///M:#ClearAllEvents#
            eventsCleared();
        }
    }
...
//        }
    ...
    ///M: add the onCreateContextMenu() and onContextItemSelected() to show context Menu. @{
    @Override
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfo) {
        Log.w(TAG, "AgendaFragment: onCreateContextMenu()" );
        android.widget.AdapterView.AdapterContextMenuInfo m = (android.widget.AdapterView.AdapterContextMenuInfo)(menuInfo);
        ...
        mAgendaListView.getSelectedItemPosition();
        int pos = m.position;

        Log.w(TAG, "AgendaFragment: onCreateContextMenu() pos="+pos );
        com.android.calendar.agenda.AgendaWindowAdapter.EventInfo event = adapter.getEventByPosition(pos);
        if(event == null){
            super.onCreateContextMenu(menu, view, menuInfo);
            Log.w(TAG, "onCreateContextMenu,event info is null!");
            return;
        }
        ...
        String when = Utils.formatDateRange(getActivity(),begin, end, flags);
    ...
    public boolean onContextItemSelected(MenuItem item) {
        Log.w(TAG, "AgendaFragment: onContextItemSelected()" );
        HeaderViewListAdapter headerAdapter = (HeaderViewListAdapter) mAgendaListView.getAdapter();
        AgendaWindowAdapter adapter = (AgendaWindowAdapter) headerAdapter.getWrappedAdapter();
        ...
        com.android.calendar.agenda.AgendaWindowAdapter.EventInfo eventinfo = adapter.getEventByPosition(pos);
        if(eventinfo != null){
            long eventId = eventinfo.id;
            Log.d(TAG, "id:" + eventId);
            Utils.sendShareEvent(getActivity(),eventId);
        }
        else{
            Log.w(TAG, "onContextItemSelected,eventinfo is null!");
        }
        return super.onContextItemSelected(item);
    }
    ///@}
    
    ///M: #ClearAllEvents# add for clear all events feature. @{
    public void eventsCleared() {
        LogUtil.w(TAG, "eventsCleared");
        if (mAgendaListView != null) {
            mAgendaListView.resetCleanQueryInitiated();
            mAgendaListView.refresh(true);
        }
    }
    ///@}
