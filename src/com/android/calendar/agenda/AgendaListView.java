...
public class AgendaListView extends ListView implements OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> a, View v, int position, long id) {
        if (id != -1) {
            ...
            if(event != null) {
            	///M: add it to send the selected event to other apps(such as email) @{
	            if(AllInOneActivity.mAgendaReqType == AllInOneActivity.REQ_TYPE_AGENDA_CHOICE_EVENT ) {
	                Uri uri = ContentUris.withAppendedId(Uri.parse("content://com.mediatek.calendarimporter/events"), event.id);
	                Intent intent = new Intent();
	                intent.setData(uri);
	                Log.i(TAG,"onItemClick(), Email selected calendar, uri="+uri);
	                
	                ((Activity) mContext).setResult(Activity.RESULT_OK,intent);
	                ((Activity) mContext).finish();
		        ///@}
	            } else if ((oldInstanceId != mWindowAdapter.getSelectedInstanceId() ||
    ...
    /**
     * reset the mCleanQueryInitiated to trigger list adapter to do query as QUERY_TYPE_CLEAN
     */
    public void resetCleanQueryInitiated() {
        LogUtil.w(TAG, "resetCleanQueryInitiated");
        //in order to re-initialize the query, set mCleanQueryInitiated to be false.
        mWindowAdapter.mCleanQueryInitiated = false;
    }
    ///@}
}
