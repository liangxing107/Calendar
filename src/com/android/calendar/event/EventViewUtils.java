...
public class EventViewUtils {
    ...
    public static boolean addPicture(Activity activity, View view, View.OnClickListener listener,
            ArrayList<LinearLayout> items, Bitmap bitmap, String pictureName, int maxPictures,
            OnItemSelectedListener onItemSelected) {

        if (items.size() >= maxPictures) {
            return false;
        }

        LayoutInflater inflater = activity.getLayoutInflater();
        LinearLayout parent = (LinearLayout) view.findViewById(R.id.picture_items_container);
        LinearLayout pictureItem = (LinearLayout) inflater.inflate(R.layout.edit_picture_item,
                null);
        parent.addView(pictureItem);

        ImageButton pictureRemoveButton = (ImageButton) pictureItem.findViewById(R.id.picture_remove);
        pictureRemoveButton.setOnClickListener(listener);

        TextView tv_pictureName = (TextView) pictureItem.findViewById(R.id.picture_name);
        tv_pictureName.setText(pictureName);

        ImageView image = (ImageView) pictureItem.findViewById(R.id.picture_value);
        if(image != null) {
