...
public class EditEventFragment extends Fragment implements EventHandler {
    ...
    private class QueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            if (cursor == null) {
                TextView loading = (TextView) mContext.findViewById(R.id.loading_message);
                if(loading != null) {
                    loading.setText(R.string.loading_failed);
                }
                return;
            }
...
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
        ...
        mView = new EditEventView(mContext, view, mOnDone, mEvent);
        startQuery();
        ...
        View cancelActionView = mView.mBtn_saveCancle;
        cancelActionView.setOnClickListener(mActionBarListener);
        View doneActionView = mView.mBtn_saveDone;
        doneActionView.setOnClickListener(mActionBarListener);
    ...
    public static boolean mIsUseLunarDatePicker = false;
    @Override
    public void onResume(){
        super.onResume();
        mView.updateDatePickerSelection();
        mView.mEt_Title.requestFocus();
        ///M: to check whether the modified event is still exist after back here. @{
        if(mIsSaveInstanceState && mUri != null){
            ...
            if(cursor == null || cursor.getCount() == 0) {
                ...
                LogUtil.i(TAG, "the event is not exist. mUri=" + mUri);
                mContext.finish();
                return;
            } else {
                cursor.close();
        ...
        case R.id.btn_done_savePane:
            if (EditEventHelper.canModifyEvent(mModel)
                ...
                if (mView != null && mView.prepareForSave()) {
                    ...
                    && mModel.mId != -1 && mOriginalModel != null
    ...
    class Done implements EditEventHelper.EditDoneRunnable {
        ...
        public void run() {
            ...
            if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
                    ...
                    && mHelper.saveEvent(mModel, mOriginalModel, mModification, mEvent.eventStyle)) {
                int stringResource;
    ...
    public void onDestroy() {
        ...
        if (mSaveOnDetach && act != null && !mIsReadOnly
            ...
            if (!isTimeInRange(mModel)) {
                toastText(mContext.getResources().getString(R.string.valid_date_range));
            } else {
    ...
    ///@}
     ...
     //@return out of range will return false or return true.
    ///M:add for check if event time out of range@{
    public boolean isTimeInRange(CalendarEventModel mModel){
        long begin = mModel.mStart;
        long end = mModel.mEnd;
        Time time = new Time(Time.TIMEZONE_UTC);
        time.set(begin);
        int firstDay = Time.getJulianDay(time.toMillis(false), time.gmtoff);
        time.set(end);
        int endDay = Time.getJulianDay(time.toMillis(false), time.gmtoff);
        
        if(firstDay < Utils.FIRST_CALENDAR_DAY || endDay > Utils.LAST_CALENDAR_DAY){
           return false;
        }
        return true;
    }
    ...
    /** M: Dispatch on stop and on start event to mView. @{ **/
    @Override
    public void onStop() {
        Log.d(TAG, "onStop() call");
        super.onStop();
        mView.dispatchOnStop();
    }
    ...
    public void onStart() {
        ...
        super.onStart();
        mView.dispatchOnStart();
    }
    /** @} **/
    
    ///M: dismiss all spinner in the edit view after called onPause.@{
    @Override
    public void onPause() {
        super.onPause();
        
        mView.dismissAllSpinners();
    }
    ///@}
}
