...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    ///M: check the valid year range. @{
    //in week view,it will dispay 7 days.so -7 is sure can show 1970.1.1 in week view.
    private static final int FIRST_CALENDAR_DAY_WEEK_VIEW = Utils.FIRST_CALENDAR_DAY - 7;
    ///@}
    /**
    ...
    */
    ...
    private void init(Context context) {
        ...
        ///M: listen the context menu instead of long click.
        setOnCreateContextMenuListener(this);
    ...
    public void handleOnResume() {
        ...
        mFutureBgColor = COLORS[0]; // chenzhentao add
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_CENTER:
                ...
                /**M: Check valid range [1970, 2037] @{*/
                if (mSelectionMode == SELECTION_SELECTED) {
                    if (mSelectionDay <= Utils.FIRST_CALENDAR_DAY || Utils.LAST_CALENDAR_DAY < mSelectionDay) {
                        Log.v(TAG, "OUT-OF-BOUND!");
                        Toast.makeText(mContext, 
                               getContext().getResources().getString(R.string.valid_date_range),
...
//                break;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        /**M: Check valid range [1970, 2037] @{*/
        Log.v(TAG, "onKeyDown(), selectionDay=" + selectionDay);
        if (keyCode == KeyEvent.KEYCODE_DPAD_CENTER) {
            if (selectionDay <= Utils.FIRST_CALENDAR_DAY || Utils.LAST_CALENDAR_DAY < selectionDay) {
                // only show toast in onKeyUp()
                return true;
            }
        }
        /**@}*/
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                ...
                if (mSelectedEvent == null) {
                    ...
                    /**M: Check valid range [1970, 2037]. @{ */
                    if (selectionDay <= Utils.FIRST_CALENDAR_DAY) {
                        Log.v(TAG, "OUT-OF-BOUND!");
                        Toast.makeText(mContext, 
                               getContext().getResources().getString(R.string.valid_date_range),
                               Toast.LENGTH_SHORT).show();
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                ...
                if (mSelectedEvent == null) {
                    ...
                    /**M: Check valid range [1970, 2037] @{*/
                    if (Utils.LAST_CALENDAR_DAY < selectionDay) {
                        Log.v(TAG, "OUT-OF-BOUND!");
                        Toast.makeText(mContext, 
                               getContext().getResources().getString(R.string.valid_date_range),
                               Toast.LENGTH_SHORT).show();
...
//                mContext.stopProgressSpinner();
    ...
    private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas, Paint p) {
        ...
        if (mNumDays > 1) {
            ...
            p.setTextSize(dateHeaderFontSize);
            ...
            p.setTextSize(dayHeaderFontSize);
            p.setTypeface(Typeface.DEFAULT);
    ...
    private void drawGridBackground(Rect r, Canvas canvas, Paint p) {
        ...
        // p.setColor(mCalendarGridLineInnerHorizontalColor);
        // p.setColor(0xFFE4E2DC); // MODIFY 222
        p.setColor(COLORS[2]); // MODIFY 222
        p.setStrokeWidth(GRID_LINE_INNER_WIDTH);
        p.setAntiAlias(false);
        y = 0;
        linesIndex = 0;
...
//            }
    ...
    private Rect drawEventRect(Event event, Canvas canvas, Paint p, Paint eventTextPaint,
        ...
        // color = 0xFF50CCFF; // chenzhentao add
        color = COLORS[3]; // chenzhentao add
        switch (event.selfAttendeeStatus) {
            case Attendees.ATTENDEE_STATUS_INVITED:
                p.setStyle(Style.STROKE);
                break;
...
//    }
    ...
    private void doDown(MotionEvent ev) {
        /** M: If the scroller over scrolled we do not stop the fling. **/
    ...
    private void doLongPress(MotionEvent ev) {
        ...
        ///M:if LongPress in invalid(<1970 or >2036) erea,ignore it.@{
        if(mSelectionDay < Utils.FIRST_CALENDAR_DAY || mSelectionDay > Utils.LAST_CALENDAR_DAY){
            LogUtil.w(TAG, "DoLongPress in the valid time erea,ignore " + mSelectionDay);
            return;
        }
        ///}
        mSelectionMode = SELECTION_LONGPRESS;
    ...
    private void doScroll(MotionEvent e1, MotionEvent e2, float deltaX, float deltaY) {
        ...
        if (mTouchMode == TOUCH_MODE_DOWN) {
            ...
            if (absDistanceX > absDistanceY) {
                ...
                ///M: if date out of range,do not init next view@{ */
                if(!isTimeInRange(e1, e2, mBaseDate)){
                    Log.v(TAG, "doScroll TOUCH_MODE_DOWN return for date range");
                    distanceX = 0;
                    return;
                }
                ///@}
            ...
            if (distanceX != 0) {
                ...
                if(!isTimeInRange(e1, e2, mBaseDate)){
                    Log.v(TAG,
                            "doScroll TOUCH_MODE_HSCROLL return for date range");
                    mViewStartX = 0;
                    return;
                }
               ///@}
    ...
    private void doFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if ((mTouchMode & TOUCH_MODE_HSCROLL) != 0) {
            ...
            ///M:if date out of date,then return false@{
            if(!isTimeInRange(e1, e2, mBaseDate)){
                Log.v(TAG, "onFling out of range:");
                Toast.makeText(mContext,
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfo) {
        ...
        if (mNumDays == 1) {
            ...
            if (numSelectedEvents >= 1) {
                ...
                if(Utils.hasImportCalendarProvider(mContext)) {
        ...
        public boolean onMenuItemClick(MenuItem item) {
            switch (item.getItemId()) {
                ...
                case MENU_EVENT_SHARE: {
                    if (mSelectedEvent != null) {
                    	Utils.sendShareEvent(getContext(), mSelectedEvent.id);
                    }
                    break;
                }
                ///@}
    ...
    private static int getEventAccessLevel(Context context, Event e) {
        ...
        LogUtil.v(TAG, "getEventAccessLevel, query event: " + e.id);
...
//        }
    ...
    ///M: remove @Override,because we do not use LongClickListner now.
    ...
    ///M: if time is out of range return false.@{
    private boolean isTimeInRange(MotionEvent e1, MotionEvent e2,Time baseDate){
        int deltaX = (int) e2.getX() - (int) e1.getX();
        int orientation = deltaX > 0 ? -1 : 1;
        int currentStartDay = 0;
        int firstDay = 0;
        int lastDay = 0;
        int nextStartDay = 0;
        currentStartDay = Time.getJulianDay(baseDate.toMillis(false),
                baseDate.gmtoff);
        lastDay = Utils.LAST_CALENDAR_DAY;
        firstDay = mNumDays == 1 ? Utils.FIRST_CALENDAR_DAY : FIRST_CALENDAR_DAY_WEEK_VIEW;
        nextStartDay = currentStartDay + orientation * mNumDays;
        if ((nextStartDay < firstDay && deltaX > 0)
                || (nextStartDay > lastDay && deltaX < 0)) {
            return false;
        }
        return true;
    }
