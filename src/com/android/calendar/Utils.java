...
public class Utils {
    ...
    // chenzhentao 2012-7-4 add start
    public static final String EVENT_STYLE_TYPE = "event_style"; 
	public static final Uri EVENT_URI = Events.CONTENT_URI;
	private static Calendar mCalendar = Calendar.getInstance();
	public static ProgressDialog mProgressDialog;
	// chenzhentao 2012-7-4 add end
    ...
    public static void setTimeZone(Context context, String timeZone) {
        ///M:
        boolean canUseProvider = canUseProviderByUri(context.getContentResolver(), CalendarContract.CalendarCache.URI);
        if(canUseProvider){
    ...
    /**
     * Formats the given Time object so that it gives the month (for
     * example, "September").
     *
     * @param time the time to format
     * @return the string containing the weekday and the date
     */
    public static String formatOnlyMonth(Context context, Time time) {
        int flags = DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_NO_MONTH_DAY
    ...
    ///M:if time out of range,return max or min julian day@{
    public static int getValidJuLianDay(Time t){
        int juLianDay = Time.getJulianDay(t.toMillis(false), t.gmtoff);
        juLianDay = juLianDay < FIRST_CALENDAR_DAY ? FIRST_CALENDAR_DAY
                : juLianDay;
        juLianDay = juLianDay > LAST_CALENDAR_DAY ? LAST_CALENDAR_DAY
                : juLianDay;
        return juLianDay;
    }
    ///@}
    ...
    /**
     * if time out of range,return max/min date,hour/minute/second not change.
     */
    ///M:only change year field to valid.@{
    public static Time getValidTime(Time t,int validJulianDay){
    ...
    ///M:#Lunar# wether show lunar calendar{
    /**
     * @return true when select to show the lunar calendar.
     */
    public static boolean isShowLunarCalendar() {
        if (Locale.SIMPLIFIED_CHINESE.equals(Locale.getDefault()) && IS_CMCC) {
            return true;
        }
        return false;
    }
    ...
    ///M: Update the notification by broadcast
    /**
     * Update the notification via broadcast 
     * @param context context
     */
    public static void updateEventReminder(Context context) {

        LogUtil.v(TAG, "update the reminder notification.");
        Intent intent = new Intent(UPDATE_EVENT_REMINDER_ACTION);
        context.sendBroadcast(intent);
    }
    ///@}
    
    /**
	 * get total days of a month month from 0 to 11
	 * authur: chenzhentao
	 * @param year
	 * @param month
	 * @return
    ...
    public static int getDaysOfMonth(Time time) {
		...
		synchronized (mCalendar) {
			mCalendar.setTimeInMillis(time.normalize(true));
			return mCalendar.getActualMaximum(Calendar.DAY_OF_MONTH);
		}
		/*
		int month = time.month;
		if (time.month < MIN_MONTH || time.month > MAX_MONTH) {
			return -1;
		}
		if (month == SPECIAL_MONTH && isLeapYear(year)) {
			return MONTH_DAYS[month] + 1; // leap year return 29 days
		}
	...
	/**
	 * verify a year is leap year or not
	 * authur: chenzhentao 
	 * @param year
	 * @return
	 */
	...
	public static int getFirstWeekdayOfMonth(Time time) {
		if (time == null) {
			return 0;
		}
		synchronized (mCalendar) {
	...
	public static final void arrayListSort(ArrayList<Integer> sourceData) {
		int len = sourceData.size();
		int[] datas = new int[len];
		for (int i = 0; i < len; i++) {
			datas[i] = sourceData.get(i);
		}
		Arrays.sort(datas);
		sourceData.clear();
		for (int i = 0; i < len; i++) {
			sourceData.add(datas[i]);
		}
	}
	
	public static void showProgress(Context context, String title,
			String message, OnCancelListener cancelListener) {
	...
	public static final String formatToDate(Context context, long timeMillis) {
		return DateUtils.formatDateTime(context, timeMillis,
	...
	private static final Uri getEventUriById(long id) {
		return ContentUris.withAppendedId(EVENT_URI, id);
	}
	
	public static final void deleteCalendarEventById(ContentResolver contentResolver,
			long id) {
		contentResolver.delete(getEventUriById(id), null, null);
	}
}
