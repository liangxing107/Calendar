...
public class SelectCalendarsSimpleAdapter extends BaseAdapter implements ListAdapter {
    ...
    ///M: #ClearAllEvents#
    /**
     * used to set the initial status to 0 in SelectClearableCalendarsFragment. @{
     */
    public void setAllItemInvisible() {
        if(mData == null){
            LogUtil.e(TAG, "setAllItemInvisible(),mData is null,return");
            return;
        }
        for (CalendarRow data : mData) {
            data.selected = false;
        }
        notifyDataSetChanged();
    }
