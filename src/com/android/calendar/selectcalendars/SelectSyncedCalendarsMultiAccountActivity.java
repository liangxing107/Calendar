...
//TODO change to something that supports group by queries.
        ...
        ///M: to filter the pc sync account, not show in sync account list.
        String[] selectArgs = { CalendarContract.ACCOUNT_TYPE_LOCAL };
        mCursor = managedQuery(Calendars.CONTENT_URI, PROJECTION,
                Calendars.ACCOUNT_TYPE +"!=?" +"1) GROUP BY (" + ACCOUNT_UNIQUE_KEY , //Cheap hack to make WHERE a GROUP BY query
                selectArgs /* selectionArgs */,
                Calendars.ACCOUNT_NAME /*sort order*/);
        ///M:JE happend when the query is failed. @{
        if(mCursor == null){
            LogUtil.e(TAG, "The account cursor is null, can not start the activity.");
            //finish();
            findViewById(R.id.btn_done).setVisibility(View.INVISIBLE);
            findViewById(R.id.btn_discard).setVisibility(View.INVISIBLE);
            return;
        }
        ///@}
